(function (window, document) {
  ...
  function Game (elementID) {
    ...
    game.parallaxSpeed = 0.5;
    game.backgroundImage = new Image();
    game.backgroundImage.src = 'images/background.webp';
    game.backgroundX = 0;
  ...
  Game.prototype.drawWelcomePage = function () {
    ...
    this.ctx.fillText("Pressione Enter para começar...", 10, 100);
  };
  ...
  Game.prototype.start = function () {
    ...
    function tick() {
      ...
      gaps.forEach(function (gap) {
        ...
        if (!gap.passed && gap.xPos + gap.width / 2 < bird.xPos - bird.width / 2) {
          gap.passed = true;
          gaps.passed++;
        }
    ...
    function render() {
      ...
      game.ctx.fillText("Pontos: " + game.score, 10, 20);
  ...
  Game.prototype.moveBackground = function () {
    this.backgroundX -= this.parallaxSpeed;
    if (this.backgroundX <= -this.screenWidth) {
      this.backgroundX = 0;
    }
    this.ctx.drawImage(this.backgroundImage, this.backgroundX, 0, this.screenWidth, this.screenHeight);
    this.ctx.drawImage(this.backgroundImage, this.backgroundX + this.screenWidth, 0, this.screenWidth, this.screenHeight);
  };
  ...
  Game.prototype.gameover = function () {
    ...
    this.ctx.fillText("Pressione enter para jogar novamente!", 60, 150);
    this.ctx.restore();
